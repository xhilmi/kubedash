...
export function LogViewer({
  ...
  const [allLogs, setAllLogs] = useState<string[]>([]) // Store all logs for filtering
  ...
  const appendLog = useCallback(
    (log: string) => {
      ...
      // Store all logs
      setAllLogs((prevLogs) => [...prevLogs, log])
      
      // Apply filter if filterTerm is set
      if (filterTerm) {
        if (!log.toLowerCase().includes(filterTerm.toLowerCase())) {
          return
        }
      }
  ...
  // Re-filter logs when filterTerm changes
  useEffect(() => {
    if (editorRef.current && allLogs.length > 0) {
      const model = editorRef.current.getModel()
      if (model) {
        const filteredLogs = filterTerm
          ? allLogs.filter((log) =>
              log.toLowerCase().includes(filterTerm.toLowerCase())
            )
          : allLogs
        
        model.setValue(filteredLogs.join('\n'))
          ...
          <div className="flex items-center gap-2">
            ...
            <CardDescription>
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                <span>
                  {filterTerm ? `${allLogs.filter((log) => log.toLowerCase().includes(filterTerm.toLowerCase())).length}` : logCount} lines {filterTerm.length > 0 && `(filtered from ${logCount})`}
                </span>
                <ConnectionIndicator
