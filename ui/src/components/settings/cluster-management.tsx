...
export function ClusterManagement() {
  ...
  const columns = useMemo<ColumnDef<Cluster>[]>(
    () => [
      ...
      {
        id: 'type',
        accessorFn: (row) => row.inCluster ? 'In-Cluster' : 'External',
      ...
      {
        id: 'Prometheus',
        accessorFn: (row) => row.prometheusURL ? 'Yes' : 'No',
    ...
    <div className="space-y-6">
      ...
      <DeleteConfirmationDialog
        ...
        additionalNote={t(
          ...
          "This action will only remove the current cluster's configuration in Kubedash and will not delete any cluster resources."
        )}
      />
    </div>
  )
