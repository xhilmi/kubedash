...
// Helper component for truncated list with popover
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="text-left hover:underline cursor-pointer text-sm text-muted-foreground"
        >
          {truncated}
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-80 max-h-60 overflow-y-auto">
        <div className="space-y-1">
          <p className="text-sm font-medium">All items ({items.length}):</p>
...
export function RBACManagement() {
  ...
  const columns = useMemo<ColumnDef<Role>[]>(
    () => [
      ...
      {
        id: 'clusters',
        accessorFn: (row) => row.clusters.join(', '),
        header: 'Clusters',
        cell: ({ row: { original: r } }) => (
          <div className="text-sm text-muted-foreground">
            <TruncatedList items={r.clusters} />
          </div>
        ),
      },
      {
        id: 'namespaces',
        accessorFn: (row) => row.namespaces.join(', '),
        header: 'Namespaces',
        cell: ({ row: { original: r } }) => (
          <div className="text-sm text-muted-foreground">
            <TruncatedList items={r.namespaces} />
          </div>
        ),
      },

      {
        ...
        cell: ({ row: { original: r } }) => (
          <div className="text-sm text-muted-foreground">
            <TruncatedList items={r.resources} />
          </div>
        ),
      },
      {
        id: 'verbs',
        accessorFn: (row) => row.verbs.join(', '),
        header: 'Verbs',
        cell: ({ row: { original: r } }) => (
          <div className="text-sm text-muted-foreground">
            <TruncatedList items={r.verbs} />
          </div>
        ),
      },
      {
