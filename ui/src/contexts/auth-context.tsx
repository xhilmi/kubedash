...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const [sessionTimeoutMinutes, setSessionTimeoutMinutes] = useState<number>(60) // default 1 hour
  ...
  const checkAuth = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Set session timeout from server response
  ...
  // Set up session timeout based on inactivity
  useEffect(() => {
    if (!user) return
    ...
    const timeoutCheckInterval = setInterval(() => {
      ...
      const now = Date.now()
      const inactiveMinutes = (now - Number(lastActivityAt)) / (1000 * 60)

      if (inactiveMinutes >= sessionTimeoutMinutes) {
        console.log(
          `Session expired after ${sessionTimeoutMinutes} minutes of inactivity`
        )
        logout()
      }
