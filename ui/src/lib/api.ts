...
// Scale deployment API - uses fine-grained RBAC endpoint
...
// Restart deployment - uses fine-grained RBAC endpoint
...
// Edit deployment - uses fine-grained RBAC endpoint (edit verb only)
export const editDeployment = async (
  name: string,
  namespace: string,
  deployment: ResourceTypeMap['deployments']
): Promise<ResourceTypeMap['deployments']> => {
  const endpoint = `/deployments/${namespace}/${name}/edit`
  return await apiClient.put<ResourceTypeMap['deployments']>(`${endpoint}`, deployment)
}

// Rollback deployment - uses fine-grained RBAC endpoint with helm rollback
// Optionally suspends FluxCD HelmRelease before rollback to prevent auto-reconciliation
export const rollbackDeployment = async (
  ...
  revision?: number,
  suspendFlux?: boolean
): Promise<{ message: string; releaseName: string; namespace: string; suspended: boolean }> => {
  const endpoint = `/deployments/${namespace}/${name}/rollback`
  const payload: Record<string, any> = { releaseName }
  if (revision !== undefined) payload.revision = revision
  if (suspendFlux !== undefined) payload.suspendFlux = suspendFlux
...
export const suspendHelmRelease = async (
  name: string,
  namespace: string,
  releaseName: string
): Promise<{ message: string; releaseName: string; suspended: boolean }> => {
  const endpoint = `/deployments/${namespace}/${name}/suspend`
  return await apiClient.post<{
    message: string
    releaseName: string
    suspended: boolean
...
export const resumeHelmRelease = async (
  name: string,
  namespace: string,
  releaseName: string
): Promise<{ message: string; releaseName: string; suspended: boolean }> => {
  const endpoint = `/deployments/${namespace}/${name}/resume`
  return await apiClient.post<{
    message: string
    releaseName: string
    suspended: boolean
...
// Helm history and values operations
export interface HelmHistoryRevision {
  revision: number
  updated: string
  status: string
  chart: string
...
export const getHelmHistory = async (
  name: string,
  namespace: string,
  releaseName: string
): Promise<HelmHistoryRevision[]> => {
  const endpoint = `/deployments/${namespace}/${name}/helm/history?release=${releaseName}`
  return await apiClient.get<HelmHistoryRevision[]>(endpoint)
}

export const detectHelmRelease = async (
  name: string,
  namespace: string
): Promise<{ deploymentName: string; releaseName: string; detected: boolean }> => {
  const endpoint = `/deployments/${namespace}/${name}/helm/detect`
  return await apiClient.get<{ deploymentName: string; releaseName: string; detected: boolean }>(endpoint)
}

export const getHelmValues = async (
  name: string,
  namespace: string,
  releaseName: string,
  revision?: number
): Promise<Record<string, any>> => {
  let endpoint = `/deployments/${namespace}/${name}/helm/values?release=${releaseName}`
...
export const getFluxStatus = async (
  name: string,
  namespace: string,
  releaseName: string
): Promise<FluxStatus> => {
  const endpoint = `/deployments/${namespace}/${name}/flux/status?release=${releaseName}`
  return await apiClient.get<FluxStatus>(endpoint)
}

// FluxCD HelmRelease operations
...
export const getHelmReleaseHistory = async (
  name: string,
  namespace: string
): Promise<{ history: HelmRevision[] }> => {
  const endpoint = `/helmreleases.helm.toolkit.fluxcd.io/${namespace}/${name}/history`
  return await apiClient.get<{ history: HelmRevision[] }>(`${endpoint}`)
}
...
export const rollbackHelmRelease = async (
  ...
  const endpoint = `/helmreleases.helm.toolkit.fluxcd.io/${namespace}/${name}/rollback`
  return await apiClient.post<{ message: string; revision: number }>(`${endpoint}`, {
    revision,
  })
}
...
// Fetch full YAML details for a specific history record
...
export const fetchResourceHistoryDetail = async (
  resourceType: string,
  namespace: string,
  name: string,
  historyId: number
): Promise<ResourceHistoryDetail> => {
  const endpoint = `/${resourceType}/${namespace}/${name}/history/${historyId}`
  return fetchAPI<ResourceHistoryDetail>(endpoint)
}
...
export const useResourceHistoryDetail = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetchResourceHistoryDetail(resourceType, namespace, name, historyId!),
    enabled: (options?.enabled ?? true) && historyId !== null,
    staleTime: 60000, // 1 minute cache for detail
  })
}
